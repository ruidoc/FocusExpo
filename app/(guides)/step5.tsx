  ...
  const FeatureItem = ({ icon, title, desc }: { icon: string, title: string, desc: string }) => (
      <View className="flex-row items-center mb-5">
          ...
          <View className="flex-1">
              <Text className="text-base font-semibold text-foreground mb-0.5">{title}</Text>
    ...
    <SafeAreaView className="flex-1 bg-background">
      <View className="flex-1 px-6 pt-12">
        ...
        <View className="bg-card rounded-3xl p-6 border border-border shadow-sm">
            ...
            <FeatureItem icon="stats-chart-outline" title="详细统计" desc="查看每日专注与使用数据" />
        </View>
      </View>
      ...
      <View className="px-6 pb-6">
        ...
        >
          <Text className="text-muted-foreground text-sm font-medium">暂不登录，直接进入</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
