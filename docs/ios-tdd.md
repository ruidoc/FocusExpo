...
- `pauseAppLimits(payload: { minutes: number }): Promise<{ resumeAt:number }>`
  - 暂停当前专注，清空屏蔽，到点恢复。
- `setSchedulePlans(plans: PlanCfg[]): Promise<void>`
...
export type Challenge = {
  ...
  goal: {
    sessions?: number;
    minutes?: number;
    window?: { days: number; ranges?: { start: number; end: number }[] };
  };
  entryCost: number;
  reward: {
    type: 'perm' | 'temp';
...
export type RankState = {
  ...
  appsLimit: number;
  minutesLimit: number;
  next?: { needM7: number; needSR7: number };
};
```
...
2. 周期计划
   - JS 生成 `PlanCfg[]` → `setSchedulePlans`；由 `MonitorExtension` 的 `intervalDidStart/End` 应用/清理。
...
本 TDD 与《iOS PRD》一致，仅包含为落地所需的实现细节与验证项；后续若 PRD 变更（文案或流程），以 PRD 最新版本为准并同步更新此文档。
